...
import { translateText } from '$lib/clients/deeplClient';
  ...
  try {
    ...
    if (data) {
      ...
      const { data: insertedData, error: insertError } = await supabase
        .from('translations')
        .insert([
          { article_id: articleId, src_language: srcLanguage, target_language: targetLanguage, translation: cleanedTranslation, original_string: cleanedHtmlContent, gpt_model: 'deepl_translate', last_updated: lastUpdated || new Date().toISOString() }
        ])
        .select('id')
        .single();
